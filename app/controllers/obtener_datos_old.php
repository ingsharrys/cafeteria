...
$query_turnero = "SELECT id_pedido, turno, estado FROM turnero";
...
$query_mesas = "
    SELECT 
        ...
        m.fecha,
        CASE 
            WHEN c.id_pedidoc IS NOT NULL THEN 1  -- Si hay un registro en caja, se considera pagado
            ELSE 0
        END AS pagado
    FROM mesas m
    LEFT JOIN caja c ON m.id_pedido = c.id_pedidoc
    GROUP BY m.numero_mesa, m.estado, m.id_pedido
";
$stmt_mesas = $conn->prepare($query_mesas);
$stmt_mesas->execute();
$mesas = $stmt_mesas->fetchAll(PDO::FETCH_ASSOC);

// Convertir pagado a booleano explícitamente
