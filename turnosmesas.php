...
foreach ($resultados as $resultado) {
    ...
    // Añadir cada pedido a la mesa correspondiente solo si la mesa está en "espera"
    // Y si el id_pro NO está en el rango de 52 a 96
    if ($resultado['estado_mesa'] === 'espera' && ($resultado['id_pro'] < 52 || $resultado['id_pro'] > 96)) {
    ...
    <div class="row" id="mesas-container">
        <?php foreach ($mesas as $mesa): ?>
            <div class="col-md-2 mb-3">
                <!-- Botón de la mesa -->
                <button class="btn btn-estado <?php echo $mesa['estado_mesa'] === 'espera' ? 'btn-en-cocina' : 'btn-otro-estado'; ?>" onclick="actualizarEstadoMesa(<?php echo $mesa['mesa_id']; ?>, '<?php echo $mesa['numero_mesa']; ?>')" id="button-<?php echo $mesa['mesa_id']; ?>">
                    <!-- Mostrar el número de la mesa, nombre del mesero y comentario solo una vez -->
                    <p><strong>Mesa:</strong> <?php echo $mesa['numero_mesa']; ?></p>
                    <?php if ($mesa['estado_mesa'] === 'espera'): ?>
                        ...
                        <?php endforeach; ?>
                    <?php else: ?>
                        <!-- Mostrar un mensaje vacío para las mesas que no están en "espera" -->
