...
function cargarDatosTurnos(tipoSolicitud) {
    ...
    fetch(`../controllers/obtener_datos_turnos.php?tipo_solicitud=${tipoSolicitud}`)
        ...
        .then(data => {
            // 🔥 AQUI AGREGA ESTO
            const currentHash = Array.isArray(data.turnos) 
                ? data.turnos.map(t => `${t.numero_pedido}-${t.estado}-${t.pagado}`).join('|') 
                : '';

            if (currentHash === lastTurnosHash) {
                isLoading = false;
                return; // 🔥 No cambiaron, no actualizamos
            }
            lastTurnosHash = currentHash;
            // 🔥 AQUI SIGUE TU MISMO CÓDIGO NORMAL...
